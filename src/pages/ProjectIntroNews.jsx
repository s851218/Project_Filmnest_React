              ...
              <div className="row mb-3" key={item.id}>
                  <div className="col-10 mx-auto">
                      <button className="btn btn-primary w-100 fs-5 d-flex justify-content-between" type="button" onClick={() => handleCollapse(item.id)}>
                        <span>{item.title}</span> <span className="d-flex align-items-center"><span className="fs-base me-2">{getTime(item.date)}</span>{(postsIsOpen[index].isOpen) ? <i className="bi bi-chevron-up"></i> : <i className="bi bi-chevron-down"></i>}</span>
                      </button>

...
export default function AboutStudio() {
  ...
  return (
    <>
      {studioProfile &&<Helmet>
        <title>{studioProfile.groupName}</title>
      </Helmet>}
      {studioProfile && <><div className="container mt-20 pb-15">
        <div className="row justify-content-center">
          <div className="col-xl-8 col-lg-10">
            <div className="border-bottom border-primary-4 pb-8 mb-8">
              <div className="row flex-md-row flex-column">
                <div className="col-lg-4 col-md-5 text-center">
                  <img src={studioProfile.studioImageUrl} className="img-fluid object-fit-cover img-director" style={{ height: 264, width: 264 }} alt={studioProfile.groupName} />
                </div>
                <div className="col-lg-8 col-md-7">
                  <div className="d-flex flex-column h-100">
                    <h3 className="fs-6 fs-md-4 mb-3">{studioProfile.groupName}</h3>
                    <p className="fs-sm">
                        ...
                        <ul className="list-unstyled mb-0">
                          <li className="fs-sm mb-1">
                            <a href={`tel:${studioProfile.phone}`}>
                              <i className="bi bi-telephone" /> {studioProfile.phone}
                            </a>
                          </li>
                          <li className="fs-sm mb-1">
                            <a href={studioProfile.email}>
                              <i className="bi bi-envelope" /> {studioProfile.email}
                            </a>
                          </li>
                          <li className="fs-sm">
                            <i className="bi bi-clock" /> {studioProfile.startTime}~{studioProfile.endTime}
                          </li>
                        </ul>
                      </div>
                    </div>
            ...
            <div id="teamIntro">
              <h3 className="fs-7 mb-3 d-flex align-items-center">
                <span className="me-2">。{studioProfile.personResponsible}</span>
          ...
          {navItems.map((item, index) => (
            <li className="nav-item border-0" key={index}>
              ...
              >
                ...
                >
                  {item}
                  {item === "進行中" ? `(${projectsData.length})` : item === "已結案" ? `(${projectsData.finLength})` : ""}
                  <span
